...
def upload_to_vk(video_path, description="", title=""):
    ...
    try:
        ...
        # Prepare message
        message = description if description else "🔥 Crush Your Limits & Stay Disciplined\n\n#fitness #motivation #gym #workout"
        
        # Ensure message is not empty (VK requirement)
        if not message.strip():
            message = "🔥 New Fitness Motivation Video!"
        ...
        video = upload.video(
            ...
            name=title or 'Fitness Motivation Video',
...
def main():
    ...
    description = sys.argv[2] if len(sys.argv) > 2 else "Crush Your Limits & Stay Disciplined 🔥"
    title = sys.argv[3] if len(sys.argv) > 3 else "Fitness Motivation"
