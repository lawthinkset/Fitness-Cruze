...
def upload_to_instagram(video_path, caption, is_story=False):
    ...
    if not access_token:
        print("[instagram] ⚠️  Skipping Instagram upload - INSTAGRAM_ACCESS_TOKEN not set")
    ...
    # Use graph.facebook.com endpoint which works with both token types
    ...
    try:
        ...
        if not is_story:
            ...
            # IMPORTANT: share_to_feed=false ensures Reels go to Reels tab ONLY (not main grid)
...
if __name__ == '__main__':
    ...
    if video_file.exists():
        try:
            result = upload_to_instagram(str(video_file), "Crush your limits and stay disciplined. 🔥 #fitness #motivation #gym #workout")
